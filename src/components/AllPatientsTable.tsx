...
const PatientsTable: React.FC = () => {
  ...
  const showDeleteConfirmation = (patientId: number) => {
  ...
  const togglePopup = (patientId: number) => {
    ...
    setPopupOpenMap((prevMap) => ({
      ...prevMap,
      [patientId]: !prevMap[patientId],
    }));
  };

  // Function to close the popup for a specific patient
  const closePopup = (patientId: number) => {
    setPopupOpenMap((prevMap) => ({
      ...prevMap,
      [patientId]: false,
    }));
  ...
  const url = "https://madad.onrender.com/api/admin/patient/get";
  ...
  const handleSearch = async (e) => {
    ...
    try {
      ...
      const url = "https://madad.onrender.com/api/admin/patient/search";
  ...
  const handleEdit = (patientId: number ) => {
    ...
    const url = `https://madad.onrender.com/api/admin/patient/update/${patientId}`;
    axios
    .put(url, formdata, {
      headers: {
        'Authorization': `Bearer ${token}`,
      ...
      // fetch the patients data table again
      fetchData();
       
    
        setPatientsData((prevPatients) => [...prevPatients, response.data.patient]);


        setSelectedPatient(null); // Close the options menu
      })
    } catch (error) {
      toast.error("Add the patient Image, try again!", {
        position: toast.POSITION.TOP_RIGHT,
        autoClose: 3000, // Adjust the duration as needed
      });
      // setLoading(false);
  ...
  const handleDelete = (patientId: number) => {
    ...
    const config = {
      method: 'delete',
      url: `https://madad.onrender.com/api/admin/patient/delete/${patientId}`,
      headers: {
        'Authorization': `Bearer ${token}`, // Include the bearer token in the Authorization header
      },
    };
    ...
    axios(config)
      .then(function (response) {
        ...
        setPatientsData((prevPatients) => prevPatients.filter((patient) => patient._id !== patientId));
  ...
  const toggleOptions = (patient: Patient) => {
  ...
  const handleFilter = (option: string) => {
    ...
    // map over the patientsData and filter using the names of the patients
    if (option !== '') {
      ...
      <div className="flex gap-2">
        ...
        <div className="relative">
          ...
          {filterDropdownVisible && (
            <div className="absolute top-12 left-0 bg-white border border-stroke rounded-b-sm shadow-lg dark:bg-boxdark">
              <ul className="py-2">
                {/* map over the patientsData and list the patient names as dropdown options */}
                {patientsData.map((patient) => (
                  <li
                    key={patient._id}
        ...
        <table className="min-w-full">
          <thead>
            <tr className="border-b border-stroke dark:bg-meta-4">
              ...
              <th className="p-2.5 xl:p-5 text-sm font-medium text-left uppercase">Subpatients</th>
          ...
          <tbody>
            ...
            {patientsData.map((patient, index) => (
              <tr key={patient._id} className={`border-b border-stroke dark:border-strokedark ${index === 0 ? 'rounded-t-sm' : ''}`}>
                ...
                <td className="p-2.5 xl:p-5 ">
                  {patient.service.length > 1 ? (
                    <div className="flex flex-col gap-1">
                      {patient.service.map((service) => (
                        <span>
                          {service.name},
                        </span>
                      ))}
                ...
                <td className="p-2.5 xl:p-5 ">{formatDatetime(patient.createdAt)}</td>
                <td className="p-2.5 xl:p-5 ">{formatDatetime(patient.updatedAt)}</td>
                <td className="p-2.5 xl:p-5 ">
                  <div className="flex flex-row gap-4">
                  <button 
                        onClick={() => togglePopup(patient._id)}                      
                        className="rounded bg-primary py-2 px-3 text-white hover:bg-opacity-90">
                      Edit
                    </button>
                    {popupOpenMap[patient._id] && (
                            <div
                              ref={popup}
                              ...
                              <div
                                ...
                                >              
                                    ...
                                    <div className="flex justify-end">
                                      <button
                                        onClick={() => closePopup(patient._id)}
